...
class Poselib_g1:
    ...
    def get_bvh_channels(self, skm):
        ...
        # root_translation = 100 * np.stack([root_translation[:, 0], root_translation[:, 1], root_translation[:, 2]], axis=-1)

        # if vis in blender
        root_translation = 100 * np.stack([root_translation[:, 0], root_translation[:, 2], -root_translation[:, 1]], axis=-1)
        # root_translation = 100 * np.stack([-root_translation[:, 0], root_translation[:, 2], root_translation[:, 1]], axis=-1)
        
        # frame = skm.global_rotation.shape[0]
        # pose_quat_global = (R.from_quat(root_rotation) * R.from_quat([0.5, 0.5, 0.5, 0.5]).inv()).as_quat().reshape(-1, 4)  
        # new_sks = SkeletonState.from_rotation_and_root_translation(skm.skeleton_tree , torch.from_numpy(pose_quat_global), torch.from_numpy(root_translation), is_local=False)
        # pose_quat = new_sks.local_rotation.numpy()
        ...
        for frame in range(local_rotation.shape[0]):
            ...
            for i in range(local_rotation.shape[1]):
                if i == 0:
                    ...
                    # rot_xyz = [-rot_xyz[0], rot_xyz[1], rot_xyz[2]]
                else:
...
def main_debug():
    ...
    output_dir = '/home/nuc/MoBox/pose2bvh/debug' # '/home/nuc/Datasets/CMP/bvh' #
    bvh_file =  'g1_nl_debug.bvh'
