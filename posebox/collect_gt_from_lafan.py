...
def single_file():
    ...
    file2 = '/home/nuc/MoBox/unitree-LAFAN1/g1/walk1_subject1.csv'

    data2 = np.genfromtxt(file2, delimiter=',')

    ## GET DATA
    root_traj_2 = data2[:, 0:3]
    root_quat_2 = data2[:, 3:7]
    dof_pose_2 = data2[:, 7:36]

    ## Convert to Quat
    root_translation, local_rotation = convert_pose_to_quat(root_traj_2, root_quat_2, dof_pose_2)
...
def multi_files():
    ## LOAD PKL
    # input_dir = '/home/nuc/MoBox/pose2bvh/lafan'
    input_dir = '/home/nuc/MoBox/unitree-LAFAN1/g1'
    output_file = '/home/nuc/MoBox/deep-motion-editing/retargeting/datasets/CMP_finetune/g1.npy'
    output_file = '/home/nuc/MoBox/deep-motion-editing/retargeting/datasets/CMP_finetune/smpl.npy'


    pkl_files = glob.glob(os.path.join(input_dir, '**', '*.csv'), recursive=True)
    pkl_files = sorted(pkl_files)
    list_of_dicts = []

    with open('/home/nuc/MoBox/deep-motion-editing/retargeting/datasets/CMP_finetune/all_list.txt', 'w', encoding='utf-8') as f:
        for item in pkl_files:
            f.write(item + '\n')

    # 遍历所有 .npy 文件并进行转换
    for pkl_file in pkl_files:
        data1 = np.genfromtxt(pkl_file, delimiter=',')
        if data1.shape[0] >= 3000:
            data1 = data1[:3000, :]

        ## GET DATA
        root_traj_1 = data1[:, 0:3]
        root_quat_1 = data1[:, 3:7]
        dof_pose_1 = data1[:, 7:36]

        ## Convert to Quat
        root_translation, local_rotation = convert_pose_to_quat(root_traj_1, root_quat_1, dof_pose_1)
