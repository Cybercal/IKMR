...
def quat_from_expmap(expmap):
    # Reference: https://github.com/facebookresearch/fairmotion/blob/main/fairmotion/ops/conversions.py
    theta = torch.linalg.norm(expmap, axis=-1, keepdim=True)
    v = expmap / theta
    v[theta[..., 0] < 1e-8] = 0.0
...
def single_file():
    ...
    file1 = '/home/nuc/MoBox/pose2bvh/data/g1_walk.pkl'
    # file2 = '/home/nuc/MoGym/PBHC/logs/icra_policy/558/metrics/ckpt_59000/tmp.pkl'

    data1 = joblib.load(file1)
    # data2 = joblib.load(file2)
    data1 = np.array(data1['frames'])
    root_traj = data1[:, :3]
    root_rot = data1[:, 3:6]
    root_quat = quat_from_expmap(torch.tensor(root_rot))
    joint_pos = data1[:, 6:]  # (frames, 29)

    ## Recover DATA
    num_frames = data1.shape[0]

    ## Convert to Quat
    root_translation, local_rotation = convert_pose_to_quat(root_traj, root_quat, joint_pos)
    ...
    # print(f"Final converted to {output_file}")
    ...
    if False:
        ...
        # 绘制第一条轨迹
        ax.scatter(root_traj[:, 0], root_traj[:, 1], root_traj[:, 2], c='blue', label='retarget_traj', alpha=0.7)

        # 绘制第二条轨迹
        # ax.scatter(root_traj[:, 0], root_traj[:, 1], root_traj[:, 2], c='red', label='simulated_traj', alpha=0.7)
