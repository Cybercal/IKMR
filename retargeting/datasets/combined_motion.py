    ...
    def __init__(self, args, datasets_groups):
        ...
        for j, datasets in enumerate(datasets_groups):
            ...
            for i, dataset in enumerate(datasets):
                ...
                # get modified datasets (mean,var part)
                if j == 0:
                    var = torch.cat((var[:, :48, :], var[:, 56:, :]), dim=1)
                    mean = torch.cat((mean[:, :48, :], mean[:, 56:, :]), dim=1)
                    new_offset = torch.cat((new_offset[:, :13, :], new_offset[:, 15:, :]), dim=1)
        ...
        for j, datasets in enumerate(all_datas):
            ...
            # get modified datasets (motion part)
            if j == 0:
                motions = torch.cat((motions[:, :48, :], motions[:, 56:, :]), dim=1)
...
class TestData(Dataset):
    ...
    def get_item(self, gid, pid, id):
        ...
        path = './datasets/Mixamo_test/{}/'.format(character)
        ...
        if not os.path.exists(file):
            ...
            print('no g1 datasets, use a fake one')
            file = './datasets/G1_less/g1/2_2_byHand_Idle_nlpose_g1_g1_29_tpose.bvh'
        file = BVH_file(file)
        ...
        # get modified datasets (motion part)
        if gid == 0:
            motion = torch.cat((motion[:48, :], motion[56:, :]), dim=0)
